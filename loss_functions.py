...
def intensity_loss_objectness(gen_frames, gt_frames, l_num, objectness_attention):
    """
    Calculates the sum of lp losses between the predicted and ground truth frames.
    ...
    @return: The lp loss.
    """
    return tf.reduce_mean(tf.abs((gen_frames - gt_frames) ** l_num) * objectness_attention)


def gradient_loss(gen_frames, gt_frames, alpha,objectness_attention):
    """
    ...
    grad_diff_x = tf.abs(gt_dx - gen_dx) * objectness_attention
    grad_diff_y = tf.abs(gt_dy - gen_dy) * objectness_attention
