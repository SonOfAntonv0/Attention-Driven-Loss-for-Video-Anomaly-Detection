...
with tf.compat.v1.variable_scope('discriminator', reuse=None):
    fake_logits, fake_outputs = discriminator(inputs=test_outputs)
...
with tf.compat.v1.Session(config=config) as sess:
    ...
    def inference_func(ckpt, dataset_name, evaluate_name):
        ...
        for video_name, video in videos_info.items():
            ...
            length = video['length']
            total += length
            psnrs = np.empty(shape=(length,256,256,3), dtype=np.float32)

            for i in range(num_his, length):
                video_clip = data_loader.get_video_clips(video_name, i - num_his, i + 1)
                psnr,out ,fkout= sess.run([test_psnr_error,test_outputs,fake_outputs],
                                feed_dict={test_video_clips_tensor: video_clip[np.newaxis, ...]})
                psnrs[i] = np.squeeze(psnr)
