...
from utils import DataLoader, load, save, psnr_error, diff_gt
...
with tf.Session(config=config) as sess:
    ...
    def inference_func(ckpt, dataset_name, evaluate_name):
        ...
        for video_name, video in videos_info.items():
            ...
            imgs = np.empty(shape=(length,4,1,256,256,3), dtype=np.float32)


            for i in range(num_his, length):
                video_clip = data_loader.get_video_clips(video_name, i - num_his, i + 1)
                test_diff_img,test_log_img,psnr,out, gt = sess.run([test_diff,test_log,test_psnr_error,test_outputs,test_gt],
                                feed_dict={test_video_clips_tensor: video_clip[np.newaxis, ...]})
                psnrs[i] = psnr

                imgs[i] = np.array([test_diff_img,test_log_img,out, gt])


                #print('video = {} / {}, i = {} / {}, psnr = {:.6f}'.format(
                #    video_name, num_videos, i, length, psnr))

            np.save("minor_result/50.npy",imgs)
            break
