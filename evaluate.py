...
import matplotlib
import os
print(f'Path is {os.path.abspath(".")}')
...
def compute_eer(loss_file):
    ...
    print('##### optimal result and model = {}'.format(optimal_results))
    return optimal_results
def get_wrong_predict(loss_file_correct,loss_file_wrong):
    def get_score_gt(sub_loss_file,index):
    ...
    score_correct,gt = get_score_gt(loss_file_correct,0)
    score_wrong,gt = get_score_gt(loss_file_wrong,0)
    result = []
    score_correct[score_correct>0.5] = 1 
    score_correct[score_correct<0.5] = 0 
    score_wrong[score_wrong>0.5] = 1 
    score_wrong[score_wrong<0.5] = 0 
    for i in range(len(score_correct)):
        if gt[i] !=1:
            continue
        if score_correct[i] == gt[i] and score_wrong[i]!=gt[i]:
            result.append(i+DECIDABLE_IDX)
    return result
...
def calculate_psnr(loss_file):
    ...
    for file in os.listdir(loss_file):
        ...
        print('mean normal psnr = {}, mean abnormal psnr = {}, mean = {}'.format(
            mean_normal_psnr,
            mean_abnormal_psnr,
            mean)
        )
...
def calculate_score(loss_file):
    ...
    PATH='C:/Users/Venky/Documents/Jupyter/Some work v1.2/Mini Project 7th sem/Attention-Driven Loss for Anomaly Detection in Video Surveillance/Codes/'
    # the number of videos
    num_videos = len(psnr_records)
    ...
    for i in range(num_videos):
        distance = psnr_records[i]
        #plt.title('Variation of PSNR with frames')
        x=range(1,distance.shape[0]+1)
        #plt.plot(x, distance)
        #plt.ylabel('PSNR')
        #plt.xlabel('Frames')
        #plt.show()
        #with open('w/o_Attention_PSNR.npy','wb') as f:
         #   np.save(os.path.join(PATH, f),distance)
        print(f'psnr records shape is {len(psnr_records)} and 1st index is {psnr_records[0].shape} and 2nd one is {psnr_records[1].shape}')
        distance = (distance - distance.min()) / (distance.max() - distance.min())
        print(f'distance is {distance.shape}')
        scores = np.concatenate((scores, distance[DECIDABLE_IDX:]), axis=0)
        plt.title('Variation of Score S_t with frames')
        x=range(1,scores.shape[0]+1)
        plt.plot(x, scores)
        plt.ylabel('Score')
        plt.xlabel('Frames')
        plt.show()
        #with open('w/o_Attention_Score.npy','wb') as f:
         #   np.save(os.path.join(PATH, f),scores)
          #  break
        labels = np.concatenate((labels, gt[i][DECIDABLE_IDX:]), axis=0)
        j=j+distance.shape[0]
    print(f'scores is {scores}')
    print(f'psnr records shape is {len(psnr_records)} and 1st index is {psnr_records[0].shape} and 2nd one is {psnr_records[1].shape}')
    #x=range(1,num_videos+1)
...
if __name__ == '__main__':
    ...
    '''
    path1 = "/data/home/dujw/anormaly/Anomaly+objectness/code/psnrs/ped2_l_2_alpha_1_lp_1.0_adv_0.05_gdl_1.0_flow_2.0DI/model.ckpt-9900"
    path2 = "/data/home/dujw/anormaly/Anomaly+objectness/code/psnrs/ped2_l_2_alpha_1_lp_1.0_adv_0.05_gdl_1.0_flow_2.0_all1_DI/model.ckpt-9900"
    print(get_wrong_predict(path1,path2))
