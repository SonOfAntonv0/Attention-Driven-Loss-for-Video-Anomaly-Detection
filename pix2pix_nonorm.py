    ...
    with tf.variable_scope('encoder'):
        ...
        for block_id, block in enumerate(blocks):
            ...
            if block_id == 0:
                ...
                net = layers.Conv2D(block.num_filters,kernel_size=[4, 4],strides=2,activation=tf.nn.leaky_relu)(net)
...
def pix2pix_discriminator(net, num_filters, padding=2, is_training=False):
    ...
    net = layers.Conv2D(
    num_filters[0],strides=2,padding='valid',activation=tf.nn.leaky_relu,kernel_size=[4, 4])(padded(net, 'conv0'))

    end_points['conv0'] = net

    for i in range(1, num_layers - 1):
        net = layers.Conv2D(
        num_filters[i],kernel_size=[4, 4],strides=2,padding='valid',activation=tf.nn.leaky_relu)(padded(net, f'conv{i}'))
    ...
    net = layers.Conv2D(
            num_filters[-1],
            strides=1
            ,kernel_size=[4, 4],padding='valid',activation=tf.nn.leaky_relu)(padded(net, f'conv{num_layers - 1}' ))
