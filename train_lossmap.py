...
from utils import DataLoader, load, save, psnr_error, objectness_rgb_estimation
...
# edgeboxes = edge_boxes_python(os.path.expanduser('./edgeboxes/edges/cpp/external/gop_1.3/data/sf.dat'))
#im = cv2.imread('test.jpeg')
#bbs = edgeboxes.get_edge_boxes(im)
# define dataset
with tf.name_scope('dataset'):
    ...
    # the modification for tensor
    # original_batch_images = tf.identity(train_videos_clips_tensor)
    ...
    #delete edge box
    # objectness_rgb_map_train = objectness_rgb_estimation(train_gt, edgeboxes)
    ...
    #delete edge box
    # objectness_rgb_map_test = objectness_rgb_estimation(test_gt, edgeboxes)
...
if lam_lp != 0:
    ...
    lp_loss_full_image = tf.abs((train_outputs-train_gt)**l_num) 
    mask = np.load("di.npy")[0]
    mask2 = np.load("di.npy")[0]
    mask[mask>0.75] = 1 
    mask[mask<0.75] = 0 
    mask = tf.convert_to_tensor(mask,dtype=tf.float32)
    lp_loss_foreground = lp_loss_full_image / mask2  * mask * 0.75
    # lp_loss_foreground = lp_loss_full_image * lp_loss_full_image
    lp_loss_full_image = tf.reduce_mean(lp_loss_full_image)
    lp_loss_foreground = tf.reduce_mean(lp_loss_foreground)
...
if lam_flow != 0:
    ...
    #objectness_flow_mask = tf.placeholder(shape=train_gt_flow.shape, dtype=tf.float32)

    #flow_loss = tf.reduce_mean(tf.abs(train_gt_flow - train_pred_flow) * objectness_flow_mask)
...
with tf.name_scope('training'):
    #flow_loss_placeholder = tf.placeholder(dtype=tf.float32)
    #g_loss = tf.add_n([lp_loss * lam_lp, gdl_loss * lam_gdl, adv_loss * lam_adv, flow_loss_placeholder * lam_flow], name='g_loss')
...
with tf.Session(config=config) as sess:
    ...
    while _step < iterations:
        try:
            ...
            _, _g_lr, _step, _lp_loss, _lp_loss_full_image,_lp_loss_foreground,_gdl_loss, _adv_loss, _flow_loss, _g_loss, _train_psnr, _summaries = sess.run(
                [g_train_op, g_lrate, g_step, lp_loss, lp_loss_full_image,lp_loss_foreground,gdl_loss, adv_loss, flow_loss, g_loss, train_psnr_error,
                 summary_op])
            full_image_losses.append(_lp_loss_full_image)
            foreground_losses.append(_lp_loss_foreground)
            if _step % 10 == 0:
            ...
            if _step % 100 == 0:
                ...
                np.save("tmp/full_image.npy",np.array(full_image_losses))
                np.save("tmp/foreground.npy",np.array(foreground_losses))
