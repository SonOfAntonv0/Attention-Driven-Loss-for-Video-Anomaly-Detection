...
import collections
import functools
from EvoNorm2dS0 import EvoNorm2dS0
...
def upsample(net, num_outputs, kernel_size, method='nn_upsample_conv'):
    ...
    if method == 'nn_upsample_conv':
        net = tf.image.resize_nearest_neighbor(
            [kernel_size[0] * height, kernel_size[1] * width])(net)
        net = layers.Conv2D( num_outputs, [4, 4], activation=None)(net)
        net=EvoNorm2dS0(num_outputs)(net)
    elif method == 'conv2d_transpose':
        net = layers.Conv2DTranspose(
           num_outputs, [4, 4], strides=kernel_size, activation=None)(net)
        net=EvoNorm2dS0(num_outputs)(net)
    else:
    ...
    with tf.variable_scope('encoder'):
        ...
        for block_id, block in enumerate(blocks):
            ...
            if block_id == 0:
                ...
                net = layers.Conv2D(block.num_filters,kernel_size=[4, 4],strides=2,activation=None)(net)
                net=EvoNorm2dS0(block.num_filters)(net)
                ...
                net = layers.Conv2D(
                        block.num_filters, activation=None,kernel_size=[4, 4],strides=2)(net)
                net=EvoNorm2dS0(block.num_filters)(net)
    ...
    with tf.variable_scope('decoder'):
        ...
        for block_id, block in enumerate(reversed_blocks):
            ...
            if block.decoder_keep_prob > 0:
                net = layers.dropout(net, keep_prob=block.decoder_keep_prob,is_training=is_training)
            end_points['decoder%d' % block_id] = net

    with tf.variable_scope('output'):
        logits = layers.Conv2D(num_outputs, [4, 4], activation=None)(net)
        # print(logits)
...
def pix2pix_discriminator(net, num_filters, padding=2, is_training=False):
    ...
    net = layers.Conv2D(
    num_filters[0],strides=2,padding='valid',activation=None,kernel_size=[4, 4])(padded(net, 'conv0'))
    net=EvoNorm2dS0(num_filters[0])(net)

    end_points['conv0'] = net

    for i in range(1, num_layers - 1):
        net = layers.Conv2D(
        num_filters[i],kernel_size=[4, 4],strides=2,padding='valid',activation=None)(padded(net, f'conv{i}'))
        net=EvoNorm2dS0(num_filters[i])(net)
        end_points[f'conv{i}'] = net

        # Stride 1 on the last layer.
    print(f'conv{num_layers - 1}')
    net = layers.Conv2D(
            num_filters[-1],
            strides=1
            ,kernel_size=[4, 4],padding='valid',activation=None)(padded(net, f'conv{num_layers - 1}' ))
    net=EvoNorm2dS0(num_filters[-1])(net)
    end_points[f'conv{num_layers - 1}'] = net

        # 1-dim logits, stride 1, no activation, no normalization.
    logits = layers.Conv2D(1,
            strides=1,
            activation=None
            ,kernel_size=[4, 4],padding='valid',)(padded(net, f'conv{num_layers}'))
    end_points['logits'] = logits
